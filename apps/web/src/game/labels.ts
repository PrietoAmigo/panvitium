...
/**
 * Display name for an action id — used by the HUD's active-action strip and the outcome log.
 * Centralised so a new action shows its real name everywhere (the old per-site maps fell back to
 * "Suggestion", mislabelling Indagatio/Emptio). Unknown ids fall back to the raw id rather than a
 * wrong name.
 */
