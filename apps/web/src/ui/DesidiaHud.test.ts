/**
 * Render tests for the Desidia vessel and toggle (ADR-033). The vessel canvas itself is a
...
import { bn, desidiaMax, type GameState } from '@panvitium/sim';
import { useGameStore } from '../store/gameStore.js';
import { DesidiaHud } from './DesidiaHud.js';

let container: HTMLDivElement | null = null;
let root: Root | null = null;

function patch(over: { desidia?: number; desidiaActive?: boolean; acedia?: number }): void {
  const s = useGameStore.getState().state as GameState;
  useGameStore.setState({
    state: {
      ...s,
      ...(over.desidia !== undefined ? { desidia: over.desidia } : {}),
...
function render(): void {
  ...
  act(() => {
    root!.render(createElement(DesidiaHud));
  });
}

/** The vessel is the toggle: a single button wrapping the pixelated canvas. */
function vesselButton(): HTMLButtonElement {
  return container!.querySelector('.desidia-hud-vessel-btn') as HTMLButtonElement;
}
...
describe('Desidia HUD', () => {
  it('renders a labelled cluster with the floored value over the base cap', () => {
    patch({ desidia: 45.7 }); // floored to 45, no decimals
    render();
    const cluster = container!.querySelector('[role="group"]');
    expect(cluster?.getAttribute('aria-label')).toBe('Desidia');
    expect(container!.querySelector('.desidia-hud-label')?.textContent).toBe('Desidia');
    expect(container!.querySelector('.desidia-hud-value')?.textContent).toBe('45 / 120');
  });

  it('shows the Acedia-doubled cap', () => {
    patch({ desidia: 10, acedia: 180 }); // Acedia level 1 → cap ×2 = 240
    render();
    expect(container!.querySelector('.desidia-hud-value')?.textContent).toBe('10 / 240');
  });

  it('floors a fractional cap (a cap sigil) so the readout stays integer-only', () => {
    // Orias #59 lifts desidiaMaxMul, which makes the derived cap fractional (120 × 1.xxx). The
    // readout must still show whole numbers only — no decimals, the reported bug.
    const s = useGameStore.getState().state as GameState;
    useGameStore.setState({ state: { ...s, desidia: 0, sigilBindings: { 59: bn(100) } } });
    const cap = desidiaMax(useGameStore.getState().state as GameState);
    expect(Number.isInteger(cap)).toBe(false); // guard: the cap really is fractional here
    render();
    const text = container!.querySelector('.desidia-hud-value')?.textContent ?? '';
  ...
  it('paints the pixelation canvas at the design buffer resolution', () => {
    render();
    const canvas = container!.querySelector('.desidia-hud-canvas') as HTMLCanvasElement | null;
  ...
  it('is inert when there is no desidia to spend and Desidia is not already active', () => {
    patch({ desidia: 0, desidiaActive: false });
    render();
  ...
  it('stays live (not inert) while Desidia runs even with an empty pool', () => {
    patch({ desidia: 0, desidiaActive: true });
    render();
  ...
  it('renders nothing when there is no game state', () => {
    ...
    render();
    expect(container!.querySelector('.desidia-hud')).toBeNull();
  });
});
