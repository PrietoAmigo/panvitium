...
**Versioning started at v1; the current schema is v3.** Two real migrations are registered in `packages/shared/src/save/migrations/` and applied in sequence by the load path (`migrate.ts`), each with its own unit test (`migrate.test.ts`): **v1→v2** collapses the removed reprobate subtypes (summing the per-subtype counts into one integer) and drops the conversion pool / cursed-subtype field (ADR-024); **v2→v3** carries the retired per-business save onto the Mercatus system — crediting 25% of each owned business's build cost as gold, fizzling the in-flight build queue, and dropping the legacy fields (ADR-025). The original `_noop.ts` placeholder has been removed now that real migrations exercise the harness.
