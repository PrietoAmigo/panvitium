...
- **Flags and resources are shared.** The phone reads and writes the same state as the PC: the resources, `totalSinLevel`, `totalSoulsObtained`, `katabasisCount`, `maleficiaOwned`, and the narrative flags defined in 05. New phone-only flags follow the ADR-023 additive-optional pattern. The phone never silently changes a flag the email arc owns. The Fausto friendly/hostile branch is owned by `flagFCfriendly` (05); **that flag is not yet wired in the sim, so the implemented calls-in gate reads `flagFCThreatSent` as its proxy** (friendly ≡ the threat reply to Fausto #1 was never sent). One consequence to be aware of: the proxy treats the un-decided default as *friendly*, the inverse of `flagFCfriendly`'s default of 0 — see `PANVITIUM-CALLS-IN.md` "Implementation notes."
