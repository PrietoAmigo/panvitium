...
const fresh = (): GameState => createInitialState('stagnation', 0);
const withAcedia = (s: GameState, devotion: number): GameState => ({
  ...s,
  devotion: { ...s.devotion, acedia: bn(devotion) },
});

describe('stagnationMax — base 120, doubled per Acedia tier (ADR-033)', () => {
  it('is the base at Acedia 0 and doubles each Sin level', () => {
    expect(stagnationMax(fresh())).toBe(STAGNATION_BASE_MAX); // 120
    expect(stagnationMax(withAcedia(fresh(), 180))).toBe(240); // level 1 → ×2
    expect(stagnationMax(withAcedia(fresh(), 180 ** 2))).toBe(480); // level 2 → ×4
  });
});

describe('grantStagnationForOffline — 0.2 per minute, clamped (ADR-033)', () => {
  it('grants 0.2/min and caps at the (Acedia-scaled) maximum', () => {
    const s = fresh();
    // 10 minutes away = 600 s → 600 × 0.2/60 = 2 stagnation.
    expect(grantStagnationForOffline(s, 600).stagnation).toBeCloseTo(2, 6);
    // An enormous span saturates at the cap (120 base).
    expect(grantStagnationForOffline(s, 1e9).stagnation).toBe(120);
    // Acedia raises the ceiling the grant clamps to.
    expect(grantStagnationForOffline(withAcedia(s, 180), 1e9).stagnation).toBe(240);
  });

  it('is a no-op for a non-positive span or when already at the cap', () => {
    const s = fresh();
    expect(grantStagnationForOffline(s, 0)).toBe(s);
    expect(grantStagnationForOffline(s, -100)).toBe(s);
    const capped = { ...s, stagnation: 120 };
    expect(grantStagnationForOffline(capped, 1e9)).toBe(capped);
  });
});
...
describe('Desidia in the tick — accelerate the sim, drain stagnation, honest clock (ADR-033)', () => {
  ...
  it('drains 1 stagnation/second (base) and advances lastTickAt by the REAL delta, not the accelerated one', () => {
    const after = tick(seeded({ desidiaActive: true }), 1).state;
    expect(after.stagnation).toBeCloseTo(99, 6); // 1/s base drain
  ...
  it('auto-deactivates the tick it cannot pay, running that tick at normal speed', () => {
    const r = tick(seeded({ stagnation: 0.5, desidiaActive: true }), 1).state;
    expect(r.desidiaActive).toBe(false);
    expect(r.stagnation).toBe(0);
