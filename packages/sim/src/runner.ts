 ...
 *
 *   - Time-mode actions (Indagatio): no per-cycle cost; cycle duration = base / efficiency.
 *   - Cost-outcome actions (Suasio/Decimatio): each cycle pays `ceil(cost × efficiency)` up front
 *     (mirroring the player's `startAction`) and resolves `max(1, floor(efficiency))` outcome units.
 *     If the channel can't afford the next cycle it STALLS — `remaining` returns null, no partial
 *     payment, no progress — and retries on a later tick once resources allow.
 *
 * `remaining` is the seconds left on the in-flight cycle, or null when between cycles / stalled.
 * The loop pays-then-counts-down and catches up multiple cycles within one (possibly offline) delta,
 * paying each one, so offline progression and online ticks share this single code path.
 */
import {
  actionCycleCost,
  canAffordCycle,
...
export interface RunnerCycleResult {
  ...
  /** Seconds left on the current cycle, or null when between cycles or stalled (can't afford). */
...
/**
 * Advance one runner channel by `deltaSeconds`. Starts (and pays for) a cycle when `remaining` is
...
export function advanceRunnerCycles(
  ...
  for (let guard = 0; guard < MAX_CYCLES_PER_CALL; guard++) {
    if (rem === null) {
      // Start a fresh cycle — pay its per-cycle cost first. Unaffordable → stall.
      const cost = actionCycleCost(actionId, efficiency);
      if (!canAffordCycle(working, cost)) break;
    ...
    rem = null; // the next cycle must pay again
    completed = true;
