...
export const serializedGameStateSchema = z.object({
  ...
  // Desidia (ADR-033): offline-banked torpor, top-level/permanent. Additive-optional (ADR-023):
  // absent → 0 at load; omitted from the wire when 0 so fresh saves keep a minimal form.
  desidia: z.number().nonnegative().optional(),
...
export function serializeGameState(state: GameState): SerializedGameState {
  ...
  return {
    ...
    // Desidia (ADR-033): omit when zero/false so fresh and pre-feature saves keep the
    // prior wire form (ADR-023 additive-optional discipline).
    ...(state.desidia > 0 ? { desidia: state.desidia } : {}),
...
export function deserializeGameState(s: SerializedGameState): GameState {
  ...
  return {
    ...
    // Desidia (ADR-033): absent → 0 / false at load (additive-optional, ADR-023).
    desidia: s.desidia ?? 0,
