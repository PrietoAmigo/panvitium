...
**Current test count: 701** (sim 511 · shared 52 · api 11 · web 127).
...
- **Sigils** — 69 of 72 are wired (see the per-slice S1–S16 table under Status). **Haures #64** is now
  wired: the murder model was changed so murders draw across **all** subtypes (Cholerics are valid
  victims by default), and Haures biases that draw toward Cholerics. The last 3 are still blocked on a
  model decision rather than effort: Ose #57 / Orias #59 (conversion re-roll toward the minority /
  majority subtype) and Vual #47 (softening the Degenerate suicide/murder penalties). Each needs its
  mechanic confirmed before it can be wired without guessing.
...
- **The last 3 sigils** (detail in the Sigils bullet above): Ose #57 / Orias #59 (conversion re-roll
  toward the minority / majority subtype) and Vual #47 (softening the Degenerate suicide/murder
  penalties). _(Haures #64 is now wired — see Status.)_
...
- **The last 3 sigils.** Ose #57 / Orias #59 (conversion re-roll toward the minority / majority subtype
  — bias the `biasedSubtype` pick by the sheet's sqrt%) and Vual #47 (soften the Degenerate
  suicide/murder penalties via the penalty-reduction channel). _(Haures #64 shipped: the murder model now
  lets Cholerics be victims, and Haures biases the victim draw toward them.)_
- **Imperium's action time.** _Decided at 10s_ (the _Suasio_ sheet had left it "Fill Time"; was a flagged
  60s placeholder).
