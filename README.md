...
- **Opera actions** — _Suasio_ is complete: _Logismoi_ and _Imperium_ are wired with sheet-accurate tiers and Luxuria gating and now sit on the scroll beside _Suggestion_ (economy-parity slice 13). What remains is the **Decimatio pair**: _Pogrom_ (single-subtype % cull — needs a subtype picker) and _Purgatio_ (all-subtype cull). Numbers for both are in the sheet.
